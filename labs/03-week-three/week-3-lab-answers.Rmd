...
```{r setup, include=TRUE, echo = FALSE, eval = FALSE}
suppressMessages({
...
source(here("docs","labs","_common.R"))
...
```
It looks like tail and total length are positively correlated, with a pretty strong relationship. Tail and head length appear somewhat positively correlated, but there's a lot more noise and the relationship appears much weaker.
...
# (ii) head & tail:
possum %>% 
  summarize(head_tail = cor(head_l, tail_l)) %>% 
  gt()
...
```{r}
# (i) the length of possum tail affects the length of a possum's body:
lm(total_l ~ tail_l, data = possum) %>% 
  tidy %>% 
  gt()
```

The coefficient on tail length tells us that for each additional cm that a possum's tail increases, on average we expect the total length of a possum to increase by 1.24cm. 

The intercept estimate tells us that on average, possums with no tail are 41cm long. **Note** that this doesn't make sense! I imagine nearly all possums have tails...this is a case where you want to be careful about extrapolating. That $\hat\beta_0$ is helping us fit the data well _in the range of our sample_, but you shouldn't go making predictions about tail-less possums using this model.

Because the slope coefficient is $>1$, increasing tail length must be increasing body length as well. To see this, imagine that we instead found that $\hat\beta_1=1$. This would imply that increasing tail length by 1cm would simply increase $\text{total length} = \text{body length} + \text{tail length}$ by 1cm, leaving body length unchanged. Because we have $\hat\beta_1 = 1.24 > 1$, body length must also be increasing as tail length increases. 

Here are analogous results for the relationship between head length and tail length:
...
For each additional cm that a possum's tail increases, on average the height of a possum's head increases by 0.524cm. 

This coefficient is **not directly comparable** to the 1.24 estimate in the previous regression, because the outcome (i.e., dependent) variable changed. Head lengths are smaller than total lengths to begin with, so we would expect smaller effect sizes here.
...
```{r}
# regression
model_1 <- lm(total_l ~ tail_l, data = possum)

# create predictions and residuals
predictions <- possum %>% add_predictions(model_1) %>%
...
# (i) the length of possum tail affects the length of a possum's head:
lm(head_l ~ tail_l, data = possum) %>% 
  summary()
```
