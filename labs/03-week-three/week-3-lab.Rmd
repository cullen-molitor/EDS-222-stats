...
[^2]: Note: If you get a `namespace load` error for `tidyverse`, you want to remove the problem package, re install it, and then load `tidyverse` again.
...
Visualizing values of the correlation between two quantities from their scatter plot can be very difficult. Research has shown that people's perception of the strength of these relationships can be influenced by design choices like the size of the graph and the aspect ratio,[^3] among other features. Hence, to more objectively report the relationship between two variables, we use correlation, computing the "correlation coefficient" $r$.[^4] Here, we will compute correlation coefficients for the two possum relationships plotted above.

[^3]: The *aspect ratio* is the ratio of the width to the height of a figure or image.
...
(i) the length of a possum's total length and the length of possum tail: 1
(ii) the length of a possum's head and the length of possum tail: .25

<!-- -->

2.  Use the `cor()` function in `R` to calculate the correlation coefficient between the two variables in (i) and the two in (ii). Prize[^5] to whoever guess was closest!

[^5]: Prize in the form of honor and admiration.
...
possum %>% 
  summarise(total_tail = cor(head_l, tail_l)) %>% 
  gt()
```
...
```{r}
mod_total_tail <- lm(formula = total_l ~ tail_l, data = possum) 
mod_sum <- summary(mod_total_tail)
mod_sum
beta_0 <- mod_sum$coefficients[1, 1]
beta_1 <- mod_sum$coefficients[2, 1]

# mod_head_tail <- lm(formula = head_l ~ tail_l, data = possum)
# summary(mod_head_tail)
```

2.  Interpret your two coefficients, paying careful attention to units.

    -   $ \hat\beta_0 $ = `r beta_0` so the y-intercept is positive and far above 0 so the model only works for the support of the data.
    -   $ \hat\beta_1 $ = `r beta_1` so there is a positive correlation 

3.  Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$?

4.  Does your model suggest anything about the relationship tail length and body length[^6]? Why or why not?
...
[^6]: Note that the "total" length is the sum of tail length plus body length.
...
1.  Add to the `possum` dataframe a column of predicted total lengths $\hat y_i$ and a column of residuals $e_i = y_i-\hat y_i$.[^7]

```{r}
possum_pred <- possum %>% 
  add_predictions(mod_total_tail) %>% 
  mutate(resids = residuals(mod_total_tail))
```
...
```{r}
ggplot(data = possum_pred, aes(x = resids)) +
  geom_histogram(binwidth = .25)

ggplot(data = possum_pred, aes(sample = resids)) +
  geom_qq() +
  geom_qq_line()
```

3.  Compute the mean of your residuals. Is it basically zero?[^8]
...
```{r}
ggplot(data = possum_pred, aes(x = tail_l, y = resids)) +
  geom_point()
```

[^7]: **Hint:** In `dplyr` you can use `add_predictions(mod)` where `mod` is a regression model to generate a new variable containing your model's predicted values.

[^8]: **Note:** OLS gives you this one for free. The OLS regression line will always go through the middle of your data, as it aims to minimize squared errors. So, you'll always end up with sample errors that are mean zero. This doesn't necessarily mean that in your population $u$ is mean zero!
...
1.  Use the function `summary()` after the regression to calculate the $R^2$ for both the total length and tail length relationship, *and* the head length and tail length relationship.
