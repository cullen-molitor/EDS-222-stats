...
3.  Install the following packages, if they are not already installed:[^1]

[^1]: Note: This will take a minute or two!
...
4.  Load all the packages you need, plus the `_common.R` source file.[^2]

[^2]: Note: If you get a `namespace load` error for `tidyverse`, you want to remove the problem package, reinstall it, and then load `tidyverse` again.
...
Create a dataframe called `loans` that cleans up the full dataset `loans_full_schema` to a) make the `homeownership` variable lower case (all caps is intense...), and b) reorder the "levels" of each variable into a sensible order.[^4]

[^4]: Note: reordering levels with `fct_relevel()` is helpful if your variable has a natural ordering to it, as all visuals will follow the order of the factor levels.

```{r}
loans <- loans_full_schema %>% 
  dplyr::mutate(homeownership = tolower(homeownership),
                homeownership = factor(homeownership, 
                                       levels = c("rent", "mortgage", "own")),
                application_type = fct_relevel(application_type, "individual", "joint"))
levels(loans$application_type)
```

## Bar plots

A bar plot is a common way to display a single categorical variable. The left panel below shows a bar plot for the homeownership variable. In the right panel, the counts are converted into proportions, showing the proportion of observations that are in each level.[^5]

[^5]: Note: The following chunk of code calls a matrix called `IMSCOL` which contains a bunch of color hex codes for colors used frequently in the IMS book. Type `IMSCOL` in your console to see all the colors you can pick from.
...
p_count <- ggplot() +
  geom_bar(data = loans, aes(x = homeownership))
p_prop <- loans %>% 
  count(homeownership) %>% 
...
Often, we are more interested in the *proportion* of our sample that falls into each category, as opposed to the total count. Recreate the above table, but now replace in each cell the counts of observations with the proportion of each application type that comes from applicants that rent, own, or have a mortgage on their home.
...
A histogram plots the count (or proportion) of your data that fall into a set of user-specified bins.[^6]

[^6]: Note: Even if you're working with continuous data, any histogram function in `R` will bin up your continuous data into discrete bins, and then plot the proportion of your data in that bin.
...
The above plot showed us *counts* in each bin, where the bins were left unspecified.[^7] The command `breaks = seq(5, 27.5, 2.5)` creates a vector called `breaks` that goes from 5 to 25 in intervals of 2.5. Generate this vector and use it within `geom_histogram()` to create new bin widths in your histogram.

[^7]: `R` will remind you to change bin values if you leave them unspecified, since the default is rarely appropriate for your data.
...
Often, we care about *proportions* instead of counts, as described above for categorical data. Remake the last histogram (with custom break points), but now show the proportion of the data that fall into each bin instead of the total number of observations.[^8]

[^8]: Hint: Since proportions indicates probability densities, you will want to look in function documentation for "density" in a histogram.
...
1.  For each $n$ from 1 to 1,000, randomly draw a sample size of $n$ from the standard normal distribution, which we know has $\mu=0$ and $\sigma=1$.[^10]

2.  For each of your 1,000 samples, compute the sample mean $\bar x_n$.

[^10]: Hint: `rnorm(x)` is a function that randomly draws $x$ numbers from the normal distribution.
